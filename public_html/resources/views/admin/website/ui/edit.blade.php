    ...
    <section class="section main-section">
        <div class="card mb-6">
            ...
            <div class="card-content">
                <form method="POST" action="{{ route('admin.ui.update', ['id' => $ui_services->id]) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="field">
                        <label class="label">Heading</label>
                        <div class="control">
                            <input class="input" type="text" value="{{$ui_services->heading}}" name="heading" placeholder="" value="">
                        </div>
                    </div>
                    <div class="field">
                        ...
                        <div class="control">
                            <div class="select">
                                <select id="link_type_drop_downs" name="type">
                                    ...
                                    <option value="experience" {{ $ui_services->type == 'experience' ? 'selected' : '' }}>Experience</option>
                                    <option value="tools" {{ $ui_services->type == 'tools' ? 'selected' : '' }}>Tools</option>
                                    <option value="enterprises" {{ $ui_services->type == 'enterprises' ? 'selected' : '' }}>Enterprises</option>
                                    <option value="design" {{ $ui_services->type == 'design' ? 'selected' : '' }}>Design</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="description-field" class="mb-3" style="display: none;">
                        ...
                        <div id="editor-container">
                            ...
                            <div id="editor">{!! old('description',$ui_services->description) !!}</div>
                        </div>
                        <textarea name="description" id="hidden-content" style="display:none;"> {{ old('description', $ui_services->description) }}</textarea>
                    </div>
                    <div id="sub-field" class="mb-3" style="display: none;">
                        <label class="form-label" for="sub_head">{{ __('Sub Heading') }}</label>
                        <textarea class="input" name="sub_head" class="form-control" placeholder="Enter sub heading">{{$ui_services->sub_head}}</textarea>
                    </div>
